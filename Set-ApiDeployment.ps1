...
.DESCRIPTION
    ...
    can chain multiple actions in a single run:
      ...
      1. Update API_URL in frontend/index.html: shows a sub-picker with two
         ...
         frontend/index.html, preserving the rest of the file:
             ...
             a. Local proxy: writes `http://127.0.0.1:8787` (the dev URL
                ...
                `npm run dev`).
             b. Cloudflare Worker: reuses the URL stored in the
                [cloudflare] section of release_info.txt when present,
                otherwise prompts for it and saves it for next time.

         After picking, the script writes the chosen URL to the
         [cloudflare] section of release_info.txt so the next run does
         not prompt again.
      ...
      5. Deploy the Cloudflare Worker (proxy/): runs `npm install` (when
         ...
         Worker URL from the wrangler output and saves it to the
         [cloudflare] section of release_info.txt. Optionally updates
         API_URL in index.html to the new Worker URL after confirmation.
...
.EXAMPLE
    pwsh ./tools/Set-ApiDeployment.ps1 -ApiUrl 'https://tasktime-cors-proxy.algo.workers.dev'
#>
...
function Invoke-WranglerDeploy {
    ...
    # wrangler prints a line like "Published tasktime-cors-proxy (x.yy.z)"
                ...
                elseif ($ApiUrl) {
                    ...
                    elseif ($subChoice.Key -eq 'Cloudflare') {
                        ...
                        if ($url) {
                            ...
                            $url = Read-Host 'Paste the Cloudflare Worker URL (e.g. https://tasktime-cors-proxy.<user>.workers.dev)'
